...
def revert(env, selection):
	error = "OK"
	selection = "%s%s"%(env.LOGS,selection)
	error,content = openfile(selection)
	content,error=apply_ldif.ldif2dict(content)
        if error != "OK":return error
        error=apply_ldif.apply_ldif(content,env)
	reverted = re.sub(r'undo', "reverted", selection)
	os.rename(selection, reverted)
	return error
...
def selectEntry(s, env, logfiles, pos = 0):
	...
	selection = None 
	s.addstr(19,2,"[Usage: [ESC] abort [U] Undo transaction [ENTER] Show transaction]",curses.color_pair(1))
	s.addstr(4, 2,"Transaction",curses.color_pair(2))
	s.addstr(4,15,"Entity",curses.color_pair(2))
	s.addstr(4,33,"Date",curses.color_pair(2))
	s.addstr(4,60,"Type",curses.color_pair(2))
	...
	while True:
		...
		for log in logfiles:
			...
			if pos == log:
				...
				mypad.addstr(log, 31, "%s"%(date), curses.color_pair(1))
				mypad.addstr(log, 58, "%s"%(logfiles[log]['type']), curses.color_pair(1))
			else:
				mypad.addstr(log, 0, "%s"%(logfiles[log]['tid']), curses.color_pair(2))
				mypad.addstr(log, 13, "%s"%(logfiles[log]['entity']), curses.color_pair(2))
				mypad.addstr(log, 31, "%s"%(date), curses.color_pair(2))
				mypad.addstr(log, 58, "%s"%(logfiles[log]['type']), curses.color_pair(2))
		...
		if x == ord('\n') and length > 0:
			...
			s.box()	
	        	s.addstr(2, 2, "Undo transaction", curses.color_pair(3))
			s.addstr(19,2,"[Usage: [ESC] abort [U] Undo transaction [ENTER] Show transaction]",curses.color_pair(1))
			s.addstr(4,2,"Transaction  Entity        Date                       Type",curses.color_pair(2))
			s.hline(5, 2, curses.ACS_HLINE, 74)
		if pos - pad_pos > 11:pad_pos += 3
		if pos - pad_pos < 0:pad_pos -= 3
	return selection, pos

def GUI_modify_transaction(env, screen):
	...
	while True:
		...
		if selection != None:
			s.erase()
	        	s.box()
	        	s.addstr(2, 2, "Undo transaction", curses.color_pair(3))
	        	s.addstr(4, 2, "%s selected"%(selection), curses.color_pair(2))
	        	s.addstr(6, 2, "Are you sure you wish to revert above selection? [Y/N]", curses.color_pair(1))
			s.addstr(19, 2, "[Use [ESC] to abort!]", curses.color_pair(1))
	        	while True:
				y = s.getch()
				if y == 27 or y == 127:break
				if y == ord('y') or y == ord('Y'):	
					stop=True
					result = revert(env, selection)
					s.erase()
					s.box()
	        			s.addstr(2, 2, "Undo transaction", curses.color_pair(3))
					s.addstr(19, 2, "[Use any key to continue]", curses.color_pair(1))
					if result == "OK":
	        				s.addstr(4, 2, "%s reverted"%(selection), curses.color_pair(2))
	        				s.addstr(6, 2, "%s"%(result), curses.color_pair(2))
	        			else:
						s.addstr(4, 2, "revert failed: %s"%(result), curses.color_pair(2))
					s.getch()
					stop = True
					break 
				if y == ord('n') or y == ord('N'):break
		else: stop = True
		if stop:
			s.erase()
			break	
